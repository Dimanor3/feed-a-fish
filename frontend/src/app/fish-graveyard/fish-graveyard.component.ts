...
class Point3d {
  constructor(x: number, y: number, z: number, camera: { x: number; z: number; y: number; }) {
    z = z/50
...
class Boid {
  constructor(xmax: number = 100, ymax: number = 100, zmax: number = 100, vmax: number = 10) {
...
export class FishGraveyardComponent implements AfterViewInit{
  ...
  draw() {
    ...
    point = new Point3d(this.canvas.nativeElement.width, this.canvas.nativeElement.height, 0, this.camera)
    this.ctx.lineTo(point.x, point.y)
    point = new Point3d(this.canvas.nativeElement.width, this.canvas.nativeElement.height, 3, this.camera)
  ...
  ngAfterViewInit(): void {
    ...
    for(let i = 0; i < this.numSouls; i++) {
      this.boids.push(new Boid(this.canvas.nativeElement.width, this.canvas.nativeElement.height, this.depth, this.maxSpd))
    }
    if(context != null){
      this.ctx = context
  ...
  drawBoids() {
    this.boids.forEach(boid => {
      ...
      this.ctx.arc(point.x, point.y, Math.abs(this.size/((boid.z/50) + 1)), 0, 2 * Math.PI);
      this.ctx.fill();
    })
  }
    ...
    for(let i = boid.bin.x - 1; i < boid.bin.x + 1; i++) {
      for(let j = boid.bin.y - 1; j < boid.bin.y + 1; j++) {
        for(let k = boid.bin.z - 1; k < boid.bin.z + 1; k++) {
          if(this.boidBins[i] && this.boidBins[i][j] && this.boidBins[i][j][k]) {
  ...
  avoidance(boid: Boid, vx0: number, vy0: number, vz0: number) {
    if (boid.x < this.leftMargin)
      boid.vx = this.turnRate + vx0
    if (boid.x > this.rightMargin)
      boid.vx = vx0 - this.turnRate 

    if (boid.y < this.topmargin)
      boid.vy = this.turnRate + vy0
    if (boid.y > this.bottommargin)
      boid.vy = vy0 - this.turnRate

    if (boid.z < 0)
      boid.vz = this.turnRate + vz0
    if (boid.z > this.depth)
      boid.vz = vz0 - this.turnRate
  }
